...
fetch('ReportMotif.json')
	...
	.then(function(motifset) {
		...
		$(motifs).each(function(i) {
			...
			sequence_logo(document.getElementById(canvaselem), 100, 50, PPM2jseqcol(this['PWM']), options);
			sequence_logo(document.getElementById(revcompcanvaselem), 100, 50, revcompPPM(PPM2jseqcol(this['PWM'])), options);
		});

		loc_len = Object.keys(loc_dict).length;
		seq_location(document.getElementById("seq_loc"), (100*loc_len), 100, buildMotifobj([first_motif_seq], loc_dict), options);
	...
	function triggerChk() {
		...
		seq_location(document.getElementById("seq_loc"), (100*loc_len), 100, buildMotifobj(listofchkd, loc_dict), options);
